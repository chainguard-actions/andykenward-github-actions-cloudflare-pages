...
1. **Checks while uploading.** While Wrangler uploads `directory`, the action checks that `github-environment` exists and finds the pull request to comment on. If either fails, the upload is stopped and the step fails straight away, so no orphaned Cloudflare deployment is left behind. An upload still running after 30 minutes is stopped too.
...
4. **Comments** on the pull request, if there is one — see [below](#which-pull-request-gets-the-comment). Set `wrangler-comment-output: false` to leave the Wrangler output out of the comment.
...
| `wrangler-comment-output` | no       | Whether the pull request comment includes the Wrangler CLI output: `true` (default) or `false`. The `wrangler` output and the job summary always have it.                                    |
...
| `Input 'pr-number' is invalid: …`                                                            | `pr-number` must be a positive whole number no larger than 2147483647.                                                                                                                                               |
| `Input 'working-directory' is invalid: Directory not found: <dir>`                           | `working-directory` doesn't exist. Check the path is relative to the repository root.                                                                                                                                |
...
| `Wrangler: timed out after 30m`                                                              | The upload ran for 30 minutes. Check `directory` isn't far larger than intended (e.g. it includes `node_modules`), and the runner's network.                                                                         |
| `A request to the Cloudflare API (<url>) failed. …`                                          | Cloudflare rejected the request; the rest of the message gives its reason and code, e.g. `Authentication error [code: 10000]` for a token without permission. `failed: 5xx …` is a Cloudflare outage — re-run later. |
